...
class UtilityPlugin(Plugin):
    ...
    def on_help(self, event):
        ...
        ]  # LIST K o m p r e h e n s i o n # noqa
        grouped = {"GENERAL": []}
    ...
    def on_kick(self, event, user, reason):
        ...
        try:
            ...
            user_dm.send_message(
                f"You have been kicked from {event.guild.name} for **{reason}**"
            )
    ...
    def on_ban(self, event, user, reason):
        ...
        try:
            ...
            user_dm.send_message(
                f"You have been **banned** from {event.guild.name} for **{reason}**"
            )
    ...
    @Plugin.command(
        "hban", "<user:snowflake> [reason:str...]", level=CommandLevels.ADMIN
    )
    def on_hban(self, event, user, reason="No Reason Specified"):
        event.guild.create_ban(user, reason=reason)

        event.msg.reply(":ok_hand:")

    # Level Management - THIS IS TEMPORARY!
    @Plugin.command(
        "set", "<user:user> <rank:str>", group="rank", level=CommandLevels.OWNER
    )
    ...
    @Plugin.command(
        "reload", "<plugin_name:str>", group="plugin", level=CommandLevels.OWNER
    )
    ...
    @Plugin.command(
        "unload", "<plugin_name:str>", group="plugin", level=CommandLevels.OWNER
    )
